...
(function initializeMonekEmbeddedCheckout(windowObject, documentObject, jQueryInstance) {
  ...
  function getWrapperElement() {
    return documentObject.querySelector(selectors.wrapper)
  ...
  function safeDestroy(component) {
    try {
      ...
      windowObject.console?.warn?.('[monek] component destroy failed', error);
    }
  }

  function unmountComponents() {
    if (state.checkoutComponent) {
      safeDestroy(state.checkoutComponent);
      state.checkoutComponent = null;
    }
    state.mountingPromise = null;
  }

  function unmountExpress() {
    if (state.expressComponent) {
      safeDestroy(state.expressComponent);
      state.expressComponent = null;
    }
  }
  ...
  async function mountExpress(selector) {
    ...
    if (state.expressComponent) {
      if (isContainerMounted(selector)) {
        if (windowObject.console?.log) {
  ...
  async function mountComponents() {
    ...
    if (state.checkoutComponent && !isContainerMounted(selectors.checkout)) {
      unmountComponents();
    }
