 ...
 * Description: Take credit/debit card payments with Monek. (Deprecated, new version available)
 * Version: 4.0.5
 * text-domain: monek-checkout
 * Requires Plugins: woocommerce
 * License: GPLv3 or later
 * License URI: https://www.gnu.org/licenses/gpl-3.0.html
 * Tags: payment, gateway, credit card, debit card, apple pay, woocommerce
...
if (!function_exists('mcwc_initialise_monek_payment_gateway')) {
    ...
    function mcwc_initialise_monek_payment_gateway() : void
    {
        ...
        function mcwc_monekcheckout_autoloader($class_name) : void
        {
            $class_map = [
                ...
                'MCWC_ApplePayFileInstaller'         => 'Infrastructure/MCWC_ApplePayFileInstaller.php',
                'MCWC_Logger'                        => 'Infrastructure/Logging/MCWC_Logger.php',
            ];
    ...
    add_action('wp_ajax_monek_download_apple_file', function () {
        ...
        $installer = new MCWC_ApplePayFileInstaller();
    ...
    add_action('after_plugin_row_' . plugin_basename(__FILE__), function($file, $plugin_data) {
        ?>
        <tr class="plugin-update-tr active">
            <td colspan="4" class="plugin-update colspanchange">
                <div class="update-message notice inline notice-warning notice-alt">
                    <p><strong>This version of Monek Checkout plugin (4.0.x) is deprecated.</strong>  
                    A major new version (4.1.x) is available, but it includes breaking changes.  
                    Please review the upgrade <a href="https://github.com/monek-ltd/Monek.Checkout.WooCommerce/blob/main/monek-checkout/tags/4.1.0/readme.txt">guide</a> before updating.
                    You can download the latest ZIP package from the <a href="https://github.com/monek-ltd/Monek.Checkout.WooCommerce/releases">GitHub releases page</a> and upload it to your site manually.</p>
                </div>
            </td>
        </tr>
        <?php
...
add_action('plugins_loaded', 'mcwc_download_apple_file_on_plugin_update', 10);

// This function checks the stored plugin version against the current version and downloads the Apple Pay domain file if an update is detected
function mcwc_download_apple_file_on_plugin_update() {
    $current_version = '4.0.5';
    $stored_version  = get_option('mcwc_version', '0.0.0');

    if (version_compare($stored_version, $current_version, '<')) {
        $logger = new MCWC_Logger();

        $logger->info('An older plugin version detected', ['stored_version' => $stored_version, 'current_version' => $current_version]);

        try {
            $installer = new MCWC_ApplePayFileInstaller();
            $installer->downloadFile();

            update_option('mcwc_version', $current_version);

            update_option('mcwc_update_done', 1);
        } catch (\Throwable $th) {
            $logger->error('Error during Apple Pay domain file installation: ' . $th->getMessage());
        }
    }
}