...
class MCWC_ShippingAddress extends MCWC_Address
{
    public function __construct() 
    {
        ...
        $this->phone = $this->mcwc_sanitize_phone(INPUT_POST, 'shipping_phone');
        
        if (!$this->mcwc_validate_phone($this->phone)) {
            throw new InvalidArgumentException("Invalid phone number format.");
        }

        if (!$this->mcwc_validate_postcode($this->billing_postcode, $this->country)) {
